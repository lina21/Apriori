...
def GetOccurences(item, transaction):
    occurences = []
    for j in range(len(transaction)):
        sub = transaction[j]
        #compare thelength of the intersetion
        if ((len(set(sub) & set(item))) == len(item)):
            occurences.append(j)
    return occurences


#search the occurence of the itemset  within span, which is a subset of transaction
def SearchSpan(itemset, span):
    num_matches = 0
    j = 0
    for item in itemset:
        found = False
        while j < (len(span)):
            #span&item - intersection of 2 lists
            sub = span[j]
            j += 1
            if ((len(set(sub) & set(item))) == len(item)):
                found = True
...
def SupportCount(transactions, itemset, maxspan):
        ...
        for i in range(len(transactions)):
            found = False
            #find all occurences of the first itemset
            occurences = GetOccurences(itemset[0], transactions[i])
            for occurence in occurences:
                found = SearchSpan(itemset, transactions[i][occurence:occurence+maxspan+1])
                if found == True:
                    break
            if found:
                support_count += 1
